...
if [ "$1" = 'uwsgi' ]; then
    ...
    if [ x"$LOCAL_DEV" = x"yes" ]; then
      ...
      for mod in ckan ckanext-bulk ckanext-bpatheme ckanext-bpaschema ckanext-s3filestore ckanext-scheming ckanext-spatial ckanext-initiatives ckanext-ytp-request ckanext-shopping-cart ckanext-laissezpasser ckanext-drs ckanext-oidc-pkce; do
