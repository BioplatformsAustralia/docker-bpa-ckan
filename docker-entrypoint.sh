...
function defaults {
    ...
    export CKANEXT_OIDC_PKCE_LOGOUT_PATH
    ...
    export CKANEXT_OIDC_PKCE_BPA_LOGOUT_REDIRECT_URL
...
function make_config {
	...
	-e "s#@AWS_ACCESS_KEY_ID@#$AWS_ACCESS_KEY_ID#" \
        ...
        -e "s#@CKANEXT_OIDC_PKCE_LOGOUT_PATH@#$CKANEXT_OIDC_PKCE_LOGOUT_PATH#" \
        ...
        -e "s#@CKANEXT_OIDC_PKCE_BPA_LOGOUT_REDIRECT_URL@#$CKANEXT_OIDC_PKCE_BPA_LOGOUT_REDIRECT_URL#" \
...
if [ "$1" = 'uwsgi' ]; then
    ...
    if [ x"$LOCAL_DEV" = x"yes" ]; then
      ...
      for mod in ckan ckanext-bulk ckanext-bpatheme ckanext-bpaschema ckanext-s3filestore ckanext-scheming ckanext-spatial ckanext-initiatives ckanext-ytp-request ckanext-shopping-cart ckanext-laissezpasser ckanext-drs ckanext-aup ckanext-oidc-pkce-bpa; do
