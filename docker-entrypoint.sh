...
function defaults {
    ...
    export AWS_ACCESS_KEY_ID AWS_HOST_NAME_TO_S3 AWS_REGION_NAME AWS_SECRET_ACCESS_KEY AWS_BUCKET_NAME AWS_LIMITED_S3_ACCESS_KEY_ID AWS_LIMITED_S3_SECRET_ACCESS_KEY AWS_LIMITED_S3_EXPIRY_IN_SECONDS AWS_STORAGE_PATH SESSION_SECRET
...
if [ "$1" = 'uwsgi' ]; then
    ...
    if [ x"$LOCAL_DEV" = x"yes" ]; then
      ...
      for mod in ckan ckanext-bulk ckanext-bpatheme ckanext-s3filestore ckanext-scheming ckanext-spatial ckanext-initiatives ckanext-ytp-request; do
